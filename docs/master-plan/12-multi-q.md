...
5b. **Variant B (NAT-friendly) — reverse-SSH relay** (§14) — **partial: deploy tooling shipped, n8n-side server pending.** Distinct from the deferred Tailscale design under the same Variant-B label; this is the reverse-SSH path that fills the NAT-traversal gap with the same per-device PKI ergonomics as Variant C. Commit 1 (the `q/` convention, §14.5) applied to all three relay packages — done. Commit 2 (`deploy/relay-ssh/` deploy tooling: two-CA `ssh-keygen` PKI, `install.sh` / `uninstall.sh`, autossh-on-Alpine container, `n8n-server/README.md` placeholder) — **shipped 2026-04-25.** Commit 3 (`SshRelayServer` singleton in `packages/n8n-nodes` + `UnoQSshRelayApi` credential + `Transport: 'ssh-relay'` HAL wiring) — **pending.** Until Commit 3 lands the Q-side container will reconnect against the unimplemented n8n endpoint; this is intentional — it lets the PKI and rsync paths be exercised end-to-end before n8n-side code ships.
