...
| relay-mtls: container `(unhealthy)` but TLS handshakes still succeed | The healthcheck flips `(unhealthy)` 30 days before `server.pem` expires (`openssl x509 -checkend $((30*86400))` probe). Renew via `./pki/pki remove <nick> && ./pki/pki add device <nick> && ./install.sh --device <nick>`. To suppress the early warning, edit the constant in [deploy/relay-mtls/q/docker-compose.yml](deploy/relay-mtls/q/docker-compose.yml). |
| relay (any variant): container `(unhealthy)`, listener not bound | The compose-level healthcheck probes for a `LISTEN` socket on the relay port. If it's missing, the underlying daemon (socat/stunnel/autossh) crashed at startup — usually port already in use after a prior crash, bad bind address, or (relay-mtls) malformed `stunnel.conf`/cert paths. `docker compose logs --tail=50` on the Q has the actual error. |
